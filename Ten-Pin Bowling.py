...
def bowling_score(frames):
    ...
    for n in range(10):
        if rolls[roll_idx] == 10:
            total += int(rolls[roll_idx]) + int(rolls[roll_idx+1]) + int(rolls[roll_idx+2])
            roll_idx += 1
        elif int(rolls[roll_idx]) + int(rolls[roll_idx+1]) == 10:
            total += int(rolls[roll_idx]) + int(rolls[roll_idx+1])
            roll_idx += 2
        else:
            total += int(rolls[roll_idx]) + int(rolls[roll_idx+1])
            roll_idx += 2
