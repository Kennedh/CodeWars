...
Your task is not to return the array itself. Instead, you must determine the maximum possible length of a sorted array
that can remain after performing these operations optimally.
...
Disclaimer
The original task was to simply implement thanos sort and return a possible resulting array. However the user can just
simply:

thanos_sort=lambda arr: arr[0]
And since the true thanos sort is random, the user is technically correct. Therefore this kata is no longer the true
thanos sort, rather a modified version.
...
def thanos_sort(arr: List[int]) -> int:
    ...
    def dfs(i, j):
        ...
        mid = (i + j) // 2
        left_len = dfs(i, mid)
        right_len = dfs(mid + 1, j) if mid + 1 <= j else 0
...
print(thanos_sort([3,1,4,2])) # 1
print(thanos_sort([1,2,3,4])) # 4
print(thanos_sort([4,3,2,1])) # 1